...
function watchStats() {
    ...
    document.getElementsByClassName("hp")[0].insertAdjacentHTML("beforeend", "<div class='display-stats'></div>")
...
class GameAI {
    constructor(turnLeft = 10, playerNumber) {
        this.turnLeft = turnLeft
    ...
    newTurn = () => {                                                                               //Décide de lanser un nouveau tour
        if (this.turnLeft === 0 || players.filter(player => player.status != "loser").length === 1) {
            ...
            this.turnLeft -= 1
            let turn = new TurnAI(this.player)
            turn.startTurn()
            return true
        }
    }
...
class TurnAI {
    ...
    playAttack = (player, ennemy) => {
        let leFront = new Front()                                                                  //choisir l'attaque
...
//Class dédiée aux fonctions de Front

class Front {
    setInterface = (playerNumber) => {
        document.getElementsByTagName("h1")[0].remove()
        while (document.getElementsByClassName("character")[0]){
            document.getElementsByClassName("character")[0].remove()
        }
        document.body.insertAdjacentHTML("beforebegin", "<div class='character-menu'><div class='character-fight' id='Fighter'><img id='0' src='./images/fighter.png'></div><div class='character-fight' id='Paladin'><img id='1' src='./images/paladin.png'></div><div class='character-fight id='Monk'><img id='2' src='./images/monk.png'></div><div class='character-fight' id='Berzerker'><img id='3' src='./images/berserker.png'></div><div class='character-fight' id='Assassin'><img id='4' src='./images/assassin.png'></div><div class='character-fight' id='Wizard'><img id='5' src='./images/wizard.png'></div></div>")
        document.getElementById(playerNumber).style.position = "absolute"
        document.getElementById(playerNumber).style.bottom = "0"
...
//--Lancement de partie
document.getElementById("character-menu").addEventListener("click", function(playerChosenImg){
    let play = new GameAI(10, Number(playerChosenImg.target.id))
    let front = new Front()
    front.setInterface(playerChosenImg.target.id)
    console.log("--- The Game Start ! ---")
    while(play.newTurn()){
        console.log(`It's turn ${11 - play.turnLeft}`)
    }
